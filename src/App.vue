...
<template>
  <div id="app">
    <a-menu @click="menuChange" v-model="current" mode="horizontal">
      ...
      <a-menu-item key="yesturday">
        <a-icon type="mail" />昨天的单词</a-menu-item
      >
      <a-menu-item key="class">
        <a-dropdown :trigger="['click']">
          <a class="ant-dropdown-link" @click="(e) => e.preventDefault()">
            <a-icon type="mail" /> {{ currentClass ? currentClass : "班级" }}
            <a-icon type="down" />
          </a>
    ...
    >
      ...
      <a-list bordered :data-source="filterWords">
        <a-list-item slot="renderItem" :key="index" slot-scope="item, index">
          <span :style="`color: ${item.remember ? 'black' : 'red'}`">{{
            item.word
          }}</span>
          {{ item.pronunciation }}
          {{ item.explain }}
  ...
  methods: {
    ...
    async onAddClass() {
      if (!this.classList.find((item) => item === this.classname)) {
        ...
        const { data = "" } = await axios.get(
          `${window.location.href}/word-list.json`
        );
        this.wordList = { words: data, class: this.classname };
        this.currentClass = this.classname;
        await localStorage.setItem(
          `${this.classname}-word-list`,
          JSON.stringify({ words: data, class: this.classname })
        );
    ...
    async handleClassMenuClick({ key }) {
      ...
      if (name !== "add") {
        ...
        this.words = this.getDateWords(
          this.getDate(new Date().getTime() - 24 * 60 * 60 * 1000)
        );
      }
    },
    handleMenuClick({ key }) {
      ...
      if (this.currentClass) {
        ...
        if (log) {
          const index = parseInt(
            Math.random() * (this.wordList?.words?.length + 1),
            10
          );
    ...
    async unkown() {
      ...
      if (word) {
        ...
        await localStorage.setItem(
          `${this.classname}-word-list`,
          JSON.stringify({ words: this.wordList.words, class: this.classname })
        );
      }
    },
    async kown() {
